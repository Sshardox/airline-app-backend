...
def create_access_token(*, sub:str) -> str:
    return create_token("access_token",
                        lifetime = timedelta(minutes=config.settings.ACCESS_TOKEN_EXPIRE_MINUTES),
                        sub = sub)
...
async def get_current_user(db: Session = Depends(db.get_db),
                           token : str = Depends(auth2_scheme)) -> User:
